...
class AccountMove(models.Model):
    ...
    niif_amount_total_signed = fields.Monetary(string='Total NIIF Signed', compute='_compute_niif_amount', currency_field='company_currency_id')
    ...
    def _prepare_values_write(self, vals):
        ...
        if 'debit' in vals or 'credit' in vals:
            for line in self:
                ...
                values = {}
                if niif_bool == 'both':
                    values.update(niif_debit=vals.get('debit') or line.debit, niif_credit=vals.get('credit') or line.credit)
